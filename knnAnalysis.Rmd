...
We removed the columns with many missing values and columns that affect accuracy. After that, we found that the base level for identifying if a patient would survive or not was 60.4%.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
High <- as.factor(ifelse(coffee_data$num_rating > 2.5, 1,0))
coffee_data_2 <- coffee_data[,colSums(is.na(coffee_data))<3]
coffee_data_2 <- coffee_data_2[complete.cases(coffee_data_2), ]
coffee_data_2 <- cbind(coffee_data_2, High)

split <- table(coffee_data_2$High)[2] / sum(table(coffee_data_2$High))

# The base rate is 90.3% for identifying whether a coffee shop was high-rated or not.
coffee_data_2 <- coffee_data_2[, -c(1, 2, 3, 4)] # remove non-numeric
coffee_data_2 <- sapply(coffee_data_2, as.numeric)
# Scale the data
coffee_data_2 <- as.data.frame(coffee_data_2)
coffee_data_2[, -c(1, 2, 9)] <- lapply(coffee_data_2[, -c(1, 2, 9)],function(x) scale(x))
# Find correlations in the data 
...
knn_diff_k_coffee = sapply(seq(1, 21, by = 2),  #<- set k to be odd number from 1 to 21
                         function(x) chooseK(x,
                                             train_set =
                                               coffee_data_train[, -c( 8)],
                                             val_set = coffee_data_test[, -c(8)],
                                             train_class = coffee_data_train[, 8],
                                             val_class = coffee_data_test[, 8]))
...
```{r, message=FALSE, echo = FALSE, warning=FALSE}
# Try 3 nearest neighbors
coffee_15NN <-  knn(train = coffee_data_train[, -8],
               test = coffee_data_test[, -8],
               cl = coffee_data_train[, 8],
               k = 3,
               use.all = TRUE,
               prob = TRUE)
str(coffee_15NN)
View(coffee_15NN)
kNN_res = table(coffee_15NN,
                coffee_data_test$Class)
View(kNN_res)
conf_matrix_initial <- kNN_res
# conf_matrix_initial
library(caret)
...
```{r, message=FALSE, echo = FALSE, warning=FALSE}
conf_matrix <- confusionMatrix(as.factor(coffee_15NN), as.factor(coffee_data_test$Class), positive = "1", dnn=c("Prediction", "Actual"), mode = "sens_spec")
conf_matrix
conf_matrix$overall["Accuracy"]
conf_matrix$overall["Kappa"]
conf_matrix$byClass["Sensitivity"]
...
# install.packages("MLmetrics")
library(MLmetrics)
LogLoss(as.numeric(attributes(coffee_15NN)$prob), as.numeric(coffee_data_test$Class))
# #We want this number to be rather close to 0, so this is a pretty terrible result. 
# 
coffee_data_test$result = coffee_15NN
F1_Score(as.numeric(coffee_data_test$result),as.numeric(coffee_data_test$Class))
 
#coffeee_15NN$prob
#coffee15<-as.data.frame(coffee_15NN.prob)
# ?MLmetrics
#attr(coffee_15NN, prob)
# attributes(coffee_15NN)$prob

# install.packages("ROCR")
library(ROCR)

pred <- prediction(as.numeric(attributes(coffee_15NN)$prob), as.numeric(coffee_data_test$Class))
View(pred)

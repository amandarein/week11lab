...
We removed the columns with many missing values and columns that affect accuracy. After that, we found that the base level for identifying if a patient would survive or not was 80.0%. In other words, there is an 80% chance that a coffee shop is high-rated.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
## Plot k vs. accuracy to see how many neighbors to use
Based on the plot that was created, 15 was the best number of neighbors for a higher accuracy level.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
set.seed(1982)
...
## Run KNN analysis with 15 nearest neighbors and analyze the accuracy of the model
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
### Log Loss
The Log Loss score we get with this data is 5.8, which indicates that something is wrong (i.e. we have too little data?)
```{r, message=FALSE, echo = FALSE, warning=FALSE}
# install.packages("MLmetrics")
library(MLmetrics)
